...
The production API and Prometheus endpoint use HTTP Basic authentication; webhook POSTs and health checks remain public. API write requests additionally require `X-Converge-Request: console`, which the console sends automatically and API clients must provide. This prevents cross-site form submissions from exercising a browser's cached Basic credentials. Use the console credentials for API access, and configure the same credentials in any Prometheus scraper. For a TLS/SASL Kafka provider, also set `KAFKA_SECURITY_PROTOCOL=SASL_SSL`, `KAFKA_SASL_MECHANISM`, and `KAFKA_SASL_JAAS_CONFIG`. Set `REDIS_SSL_ENABLED=true` for TLS Redis. Verify the provider's required URL/port and network rules before deploying. Do not put credentials in `fly.toml` or Git. The application name can be changed there if `converge-inventory-santinomarial` is unavailable.
